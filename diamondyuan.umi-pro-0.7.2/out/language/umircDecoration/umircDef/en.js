...
exports.default = {
    ...
    exportStatic: 'If set to true or Object, all routes are exported as static pages, otherwise only one index.html is output by default',
    ...
    targets: 'Configuring the minimum version of browsers you want to compatible with, umi will automatically introduce polyfill and transform grammar',
    theme: 'The configuration theme is actually equipped with the less variable. Support for both object and string types, the string needs to point to a file that returns the configuration',
    define: "Passed to the code via the webpack's DefinePlugin , the value is automatically handled by JSON.stringify",
    ...
    extraBabelIncludes: 'Define a list of additional files that need to be babel converted, in the form of an array, and the array item is webpack#Condition',
    extraPostCSSPlugins: 'Define additional PostCSS plugins in the format of an array',
    cssModulesExcludes: 'The files in the specified project directory do not go css modules, the format is an array, and the items must be css or less files',
    copy: 'Define a list of files that need to be copied simply. The format is an array. The format of the item refers to the configuration of copy-webpack-plugin',
    proxy: 'Configure the proxy property of webpack-dev-server. If you want to proxy requests to other servers',
    sass: 'Configure options for node-sass. Note: The node-sass and sass-loader dependencies need to be installed in the project directory when using sass',
    manifest: 'After configuration, asset-manifest.json will be generated and the option will be passed to https://www.npmjs.com/package/webpack-manifest-plugin',
    ...
    cssLoaderOptions: 'Additional configuration items for css-loader.Configure the resolve.alias property of webpack',
    ...
    plugins: 'Specify the plugins. The array item is the path to the plugin and can be an npm dependency, a relative path, or an absolute path',
