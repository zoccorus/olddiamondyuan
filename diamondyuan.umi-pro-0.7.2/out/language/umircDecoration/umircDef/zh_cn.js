...
exports.default = {
    ...
    theme: '配置主题，实际上是配 less 变量。支持对象和字符串两种类型，字符串需要指向一个返回配置的文件',
    ...
    extraBabelIncludes: '定义额外需要做 babel 转换的文件匹配列表，格式为数组，数组项是 webpack#Condition',
    ...
    manifest: '配置后会生成 asset-manifest.json，option 传给 https://www.npmjs.com/package/webpack-manifest-plugin',
    ...
    plugins: '配置插件列表。数组项为指向插件的路径，可以是 npm 依赖、相对路径或绝对路径。如果是相对路径，则会从项目根目录开始找',
